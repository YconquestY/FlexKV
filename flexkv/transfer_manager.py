...
class TransferManager:
    ...
    def initialize_transfer_engine(self) -> None:
        ...
        if self.storage_engine.has_indexer_storage_handle(DeviceType.CPU):
            indexer_gpu_handles = {}
            for device_id in sorted(self.all_gpu_blocks.keys()):
                if self.storage_engine.has_indexer_storage_handle(DeviceType.GPU, device_id):
                    dp_client_id = self.gpu_client_mapping[device_id]
                    if dp_client_id not in indexer_gpu_handles:
                        indexer_gpu_handles[dp_client_id] = []
                    indexer_gpu_handles[dp_client_id].append(
                        self.storage_engine.get_indexer_storage_handle(DeviceType.GPU, device_id))
        indexer_cpu_handle = (
            self.storage_engine.get_indexer_storage_handle(DeviceType.CPU)
            if self.storage_engine.has_indexer_storage_handle(DeviceType.CPU)
            else None
        )
        indexer_ssd_handle = (
            self.storage_engine.get_indexer_storage_handle(DeviceType.SSD)
            if self.storage_engine.has_indexer_storage_handle(DeviceType.SSD)
            else None
        )
        indexer_remote_handle = (
            self.storage_engine.get_indexer_storage_handle(DeviceType.REMOTE)
            if self.storage_engine.has_indexer_storage_handle(DeviceType.REMOTE)
            else None
        )
